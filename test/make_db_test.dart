...
void main() {
  test('Build dictionary.db', () async {
    // Initialize FFI
    sqfliteFfiInit();
    databaseFactory = databaseFactoryFfi;

    print('Building dictionary.db in test...');

    final dbFile = File('assets/dictionary.db');
...
Future<void> _ingestWordData(Database db) async {
  ...
  final dataDir = Directory('dictionary/data');
  if (!dataDir.existsSync()) {
    print('Error: dictionary/data directory not found!');
    return;
  }
  ...
  await db.transaction((txn) async {
    ...
    Future<void> insertWord(String wordRaw, List<String> categories) async {
      ...
      if (res.isNotEmpty) {
        ...
        wordId = await txn.insert('words', {'text': word, 'is_common': 1});
        wordCount++;
      }
...
Future<void> _ingestThesaurus(Database db) async {
  ...
  final file = File('dictionary/other/en_thesaurus.jsonl');
  if (!file.existsSync()) {
    print('Thesaurus file not found, skipping.');
    return;
  }
      ...
      for (final line in batchLines) {
        ...
        try {
          ...
          if (res.isNotEmpty) {
            ...
            wordId = await txn.insert('words', {'text': word, 'is_common': 0});
          }
